...
int odbierz_z_timeoutem(int msgid, void* msg, int rozmiar, long typ, int max_prob) {
    for (int i = 0; i < max_prob; i++) {
        ...
        if (errno != ENOMSG) {
            return 0; // Błąd inny niż brak wiadomości
        }
...
void pasazer_run(int id, int shmid, int semid, int msgid, int typ, pid_t pid_dziecka) {
    ...
    // Opiekun przesyła bilet dziecku
    if (typ == TYP_OPIEKUN) {
    ...
    // Pętla oczekiwania na autobus
    int wszedlem = 0;
    while (!wszedlem) {
        // Blokada semafora drzwi (chyba, że VIP lub Dziecko - oni nie czekają w kolejce)
        if (typ != TYP_VIP && typ != TYP_DZIECKO) zablokuj_semafor(semid, moje_drzwi);
        
        zablokuj_semafor(semid, SEM_MUTEX);
        data = dolacz_pamiec(shmid);

        // Sprawdzenie czy dworzec jest nadal otwarty
        if (data->dworzec_otwarty == 0) {
        ...
        // obsługa priorytetu VIP (jeśli są VIPy, zwykli pasażerowie nie wchodzą)
        ...
        if (data->autobus_obecny == 1) {
            ...
            // Opiekun sprawdza czy są 2 miejsca
            if (typ == TYP_OPIEKUN) {
            ...
            odlacz_pamiec(data); 
            odblokuj_semafor(semid, SEM_MUTEX); // Zwolnienie mutex na czas rozmowy z kierowcą

            if (czy_probowac) {
                if (typ == TYP_DZIECKO) {
                    // Dziecko czeka na sygnał od opiekuna
                    BiletMsg wolanie;
                    if (odbierz_z_timeoutem(msgid, &wolanie, sizeof(BiletMsg) - sizeof(long), getpid(), 50)) {
                         zgoda = 1; 
                    }
                } else {
                    // Reszta pasażerów komunikuje się bezpośrednio z kierowcą
                    BiletMsg bilet;
                    ...
                    // Czeka na odpowiedź od kierowcy
            ...
            if (zgoda) {
                ...
                // Synchronizacja dla opiekuna i dziecka
                if (typ == TYP_DZIECKO) {
                ...
                if (typ == TYP_OPIEKUN) {
                    // Opiekun informuje kierowcę o wejściu dziecka
                    BiletMsg bilet;
                    bilet.mtype = KANAL_KONTROLA;
                    bilet.pid_nadawcy = getpid();
                    ...
                    // Czeka na zgodę od kierowcy na wejście dziecka
                    BiletMsg odp;
                    if (odbierz_z_timeoutem(msgid, &odp, sizeof(BiletMsg) - sizeof(long), getpid(), 50)) {
                        ...
                        // Czeka na potwierdzenie od dziecka
                        BiletMsg potwierdzenie;
        ...
        // Zwolnienie semafora drzwi, jeśli nie wszedł
        if (typ != TYP_VIP && typ != TYP_DZIECKO) odblokuj_semafor(semid, moje_drzwi);
...
// Logika procesu autobusu/kierowcy
void autobus_run(int id, int shmid, int semid, int msgid) {
    ...
    while (1) {
        ...
        // Jeśli w pętli wewnętrznej wykryto koniec pracy, przerwanie też głównej pętli
        if (czy_koniec_pracy) {
            break; 
        }
        ...
        while(time(NULL) - start < T_ODJAZD && !wymuszony_odjazd) {
            ...
            // Nieblokujące sprawdzanie komunikatów od pasażerów
            if (odbierz_komunikat_bez_blokowania(msgid, &bilet, rozmiar, KANAL_KONTROLA)) {
                ...
                if (mozna) {
                    ...
                    odp.typ_pasazera = data->liczba_pasazerow; // Informacja o stanie dla pasażera
                    odp.pid_nadawcy = data->liczba_rowerow;

                    // Jeśli opiekun, ustawienie flagi oczekiwania na dziecko
        ...
        // Odrzucenie wszystkich, chyba że czekanie na dziecko opiekuna, który właśnie wszedł
        while (1) {
            BiletMsg bilet;
            int rozmiar = sizeof(BiletMsg) - sizeof(long);
